...
## 7.2. チュートリアル-RaspberryPiの初期セットアップ
...
- AWSからダウンロードした２つのファイル接続キット:`{core device name}-connectionKit.zip`、インストーラー:`greengrass-nucleus-latest.zip`をRaspberryPi内のディレクトリに配置します
  - `インストーラー`：`~/GreengrassInstaller`配下に解凍。GUIでは、ファイル上で右クリックし`ここでファイルを展開`をクリックするだけででOK。
  - `接続キット`：`/greengrass/v2`配下に解凍。GUIでは権限不足になるため以下コマンドを実行
...
TODO
      ...
      iotCredEndpoint: "{your endpoint}" # greengrass-nucleus-latest.zip内の config.yamlに記載されています
```
...
## 7.3. チュートリアル-S3バケットとサンプルファイルを作成
...
### 7.6.4. デプロイ-Step4.詳細設定を構成

- 初期設定のままでOKです

### 7.6.5. デプロイ-Step5.レビュー
...
- コード自体に特別な実装は無し。一般的な実装と同じく`import`を実装するだけ
...
- [Step3 コンポーネントを設定](#デプロイ-step3コンポーネントを設定)にてマイコンポーネントの設定変更にアクセス
