---
title: " 現地訪問必須だったRaspberryPiの管理地獄に終止符-AWS Greengrass"
emoji: "😎"
...
- `AWS IoT` > `Greengrass` > `コアデバイス`へアクセスし
...
| ソフトウェアランタイム                           | Greengrass nucleus                                     | RaspberryPi Pico等の小メモリ機は[liteを選択する](https://docs.aws.amazon.com/ja_jp/greengrass/v2/developerguide/greengrass-nucleus-lite-component.html) |
| オペレーティングシステム                         | Linux                                                  | 端末に合わせて設定する                                                                                                                                            |
| デバイスのセットアップ方法                       | `インストーラーのダウンロードでデバイスをセットアップ` | 既にRaspberryPiが起動している場合利用する                                                                                                                 |
| AWS IoT モノ                                     | クリックする                                           | `全てのデバイス`にモノが作成されます                                                                                                                          |
| 接続キットをダウンロード                         | クリックする                                           | 後からダウンロードできないので注意                                                                                                                |
| インストーラーをダウンロード                     | クリックする                                           | Greengrass nucleus ランタイムインストールファイル一式                                                                                |
| ダウンロードをデバイスに転送してセットアップする | コピーしておく                                 | RaspberryPiで実行するコマンドです                                                                                                                                 |

- 作成を完了させます

## 6.2. チュートリアル-RaspberryPiの初期セットアップ

- Raspberry Piでランタイム実行に必要なJavaをインストールします

```sh
sudo apt install -y openjdk-11-jdk
```

- コアデバイスの作成過程でダウンロードされた２つのファイルは以下のようにします（マネジメントコンソールに記載されている通りです）

| ファイル種 | ファイル名 | 操作 |
| --- | --- | --- |
| インストーラー | `greengrass-nucleus-latest.zip` | `~/GreengrassInstaller`配下に解凍。GUIでは、ファイル上で右クリックし`ここでファイルを展開`をクリックする |
...
::: details 一時ファイル/greengrass/v2/{id}を開くことがでないエラー
...
bucket
    └ artifacts
        └ com.example.HelloWorld(component name)
            └ 1.0.0
                └
```
...
- `AWS IoT` > `Greengrass` > `コンポーネント` へアクセスし`コンポーネントを作成`をクリック
...
１度作成したデプロイオブジェクトは再デプロイ時に、変更/更新する運用になります（デプロイの名前も更新可能ではある）
再利用の観点から、**日時情報**などは命名に使用せず、対象のデプロイターゲットが**どんなものか**を命名に使用するのが良いと思っています
...
+        "run": "python3 {artifacts:decompressedPath}/artifact/hello_world.py"
      },
      "Artifacts": [
        {
...
- `DefaultConfiguration`をアップデートした後、以降の作業を実施しないと`awsiot.greengrasscoreipc.model.UnauthorizedError`が発生することがありますのでご注意ください
  - [UnauthorizedErrorについて](https://docs.aws.amazon.com/ja_jp/greengrass/v2/developerguide/troubleshooting.html#ipc-unauthorized-error)
...
- [Step3 コンポーネントを設定](#663-デプロイ-step3コンポーネントを設定)にてマイコンポーネントの設定項目に以下のように追加設定します
...
https://docs.aws.amazon.com/ja_jp/greengrass/v2/developerguide/component-recipe-reference.html
...
- topicを限定して`resources`で指定する場合は、以下のようにTopicを指定することができます
...
- トピックのフィルターに`devices/#`と入力しサブスクライブをクリックして待つ
- 以下のようなメッセージが表示されるはずです

```json
{
  "timestamp": "2026-01-01T01:00:00.000+09:00",
  "level": "INFO",
  "message": "Hello Greengrass mqtt message"
}
```
...
:::message
本例では`GreengrassV2IoTThingPolicy`に追加しておりますがご自身でポリシーを追加する等、適切にご判断ください
:::
...
- **boto3**を使用した直接的な操作を追加アクセスキーやシークレット無しに実施することができます
...
- 本例では`GreengrassV2CoreDeviceRole`を使用しました。 使用ロールは適切にご判断ください
- 本例では、S3にIoTデバイスからファイルを直接アップロードさせるよう、以下の権限を付与します
...
- **デプロイフローの統一**: クラウド経由でのデプロイを強制することで、現場暫定処理の「野良ブランチ」発生を抑制し、ガバナンスを強化します。
...
*Greengrassでイケてるデバイス管理*

# 16. 終わりに

本記事内容のみでは、せいぜい数個～十数個程度のデバイス管理までが限界だと思います
より多くのデバイスを扱う場合は本記事では触れていない、プロビジョンやデプロイメントの効率管理が必要になるはずです
...
大変長くなってしまいましたが、自身が見返す際に`Ctrl+F`で検索することを考え１記事にしました
